...
$(function(){
  ...
  $(document).click(function(e){
    if(!$(e.target).is(".bank-ccv") || !$(e.target).closest(".bank-ccv").length){
      $(".bank-card").css("transform", "rotatey(0deg)");
      $(".bank-seccode").css("color", "var(--text-color)");
    }
    if(!$(e.target).is(".bank-expire") || !$(e.target).closest(".bank-expire").length){
      $(".bank-date_value").css("color", "var(--text-color)");
    }
    if(!$(e.target).is(".bank-number") || !$(e.target).closest(".bank-number").length){
      $(".bank-card_number").css("color", "var(--text-color)");
    }
    if(!$(e.target).is(".bank-inputname") || !$(e.target).closest(".bank-inputname").length){
      $(".bank-fullname").css("color", "var(--text-color)");
    }
  });
  
  
  //Card number input
  $(".bank-number").keyup(function(event){
    $(".bank-card_number").text($(this).val());
    number = $(this).val();
    ...
    if(selected_card != -1){
      ...
      $(".bank-bankid").attr("src", cards[selected_card].src).show();
    }else{
      html.setAttribute("style", "--card-color: #cecece");
      $(".bank-bankid").attr("src", "").hide();
    }
    
    if($(".bank-card_number").text().length === 0){
      $(".bank-card_number").html("&#x25CF;&#x25CF;&#x25CF;&#x25CF; &#x25CF;&#x25CF;&#x25CF;&#x25CF; &#x25CF;&#x25CF;&#x25CF;&#x25CF; &#x25CF;&#x25CF;&#x25CF;&#x25CF;");
    }

  }).focus(function(){
    $(".bank-card_number").css("color", "white");
  ...
  //Name Input
  $(".bank-inputname").keyup(function(){  
    $(".bank-fullname").text($(this).val());  
    if($(".bank-inputname").val().length === 0){
        $(".bank-fullname").text("FULL NAME");
    }
    return event.charCode;
  }).focus(function(){
    $(".bank-fullname").css("color", "white");
  });
  
  //Security code Input
  $(".bank-ccv").focus(function(){
    $(".bank-card").css("transform", "rotatey(180deg)");
    $(".bank-seccode").css("color", "white");
  }).keyup(function(){
    $(".bank-seccode").text($(this).val());
  ...
  //Date expire input
  $(".bank-expire").keypress(function(event){
    ...
    $(".bank-date_value").html($(this).val());
    if(event.keyCode == 8 && $(".bank-expire").val().length == 4){
      $(this).val(month);
    }
    
    if($(this).val().length === 0){
      $(".bank-date_value").text("MM / YYYY");
    }
  }).keydown(function(){
    $(".bank-date_value").html($(this).val());
  }).focus(function(){
    $(".bank-date_value").css("color", "white");
  });
});